...
//print("The value of foo is \(foo)")
// it will generate compile error beacuse foo cant be used out of the block as it would be outside of its scope
...
// x is defined in global scope so it can be accesed in the for loop.
// The foo was declared inside the loop,so it existed only during loop iteration and couldn't be accessed outside that
...
func greeting(greeting: String?, name: String) {
    ...
    greeting(greeting: "Hi there", name: "acd")
    greeting(greeting: nil, name: "abc")
